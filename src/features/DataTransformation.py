...
# My own functions developed for this project


def suggest_cutoff(avg_rep_duration, multiplier=2.5):
    """
    Suggest a lowpass filter cutoff frequency based on average rep duration.

    Parameters:
    - avg_rep_duration (float): Average duration of one rep in seconds
    - multiplier (float): How many times faster noise is compared to the signal.
                          Typical values: 2.0 to 3.0

    Returns:
    - cutoff_freq (float): Recommended cutoff frequency in Hz
    """

    # Rep frequency = how often a rep happens per second
    rep_freq = 1 / avg_rep_duration

    # Noise tends to be higher frequency, so we allow a bit more room
    # Multiplier gives a buffer above the rep frequency to preserve shape
    cutoff_freq = rep_freq * multiplier
